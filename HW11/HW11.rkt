...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname HW11) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;***** Environment Stuff *****
(define resolve
  (lambda (varName env)
    (cond
    ...
    (cond
      ...
      ((eq? (car parsed-no-code) 'let-exp)
       (let* ((new-scope (construct-scope (cadr parsed-no-code) env))
              ...
              (new-scope (construct-scope (caddr parsed-no-code) env))
              (new-env (extend-env new-scope env)))
         (run-parsed-code caddr (cdr new-env))))
...
(define sample-let-no-code '(local-vars ((a 2) (b 3)) (local-vars ((a 5)) (do-mathy-stuff + a b))))
