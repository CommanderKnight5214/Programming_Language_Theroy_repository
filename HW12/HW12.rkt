...
(define no-code-do-parser
  (lambda (no-code-do)
    (cond
      ((null? (car no-code-do)) '())
      (else (cons (no-parser (car no-code-do)) (no-code-do-parser (cdr no-code-do)))))))
             
;'(Do ((remember a 15) (remember b 17) (spit-out a) (spit-out b) (remember c (do-mathy-stuff + a b)) (spit-out c))))
...
(define no-parser
  (lambda (no-code)
    (cond
      ...
      ((eq? (car no-code) 'Do)
       (list 'do-exp (no-code-do-parser (cadr no-code))))
...
(define run-parsed-do-code
  (lambda (parsed-do-code env)
    (cond
      ((null? (car parsed-do-code)) '())
      ((eq? (car parsed-do-code) 'do-exp)
       (run-parsed-code (car parsed-do-code) env)
       (run-parsed-do-code (cdr parsed-do-code) env)))))
...
(define run-parsed-code
  (lambda (parsed-no-code env)
    (cond
      ...
      ((eq? (car no-code) 'do-exp)
       (run-parsed-do-code parsed-no-code env))
